...
export async function generateMetadata({ params }: PageProps) {
  ...
  const { frontmatter } = await getDocBySlug(slug, defaultLocale);
...
export default async function DocsPage({ params }: PageProps) {
  ...
  const { content } = await getDocBySlug(slug, defaultLocale)
  const toc = await generateTocForDocument(content)
  const navigation = await getNavigationData(currentSlug, defaultLocale)
  // MDX 파일 읽기
  const filePath = path.join(process.cwd(), 'content', defaultLocale, `${slug.join('/')}.mdx`)
